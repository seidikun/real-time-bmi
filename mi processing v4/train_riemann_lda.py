...
def run_cv_return_c_mean(cov_data, labels, all_trial_num, unique_labels, list_classes, n_folds = 10):
    best_acc      = 0
    best_c_mean   = None
    ...
    for fold in range(n_folds):
        ...
        for iclass in range(len(list_classes)):
            ...
            plt.contourf(xx, yy, dict_z[zname], alpha=0.5, levels=np.linspace(dict_z[zname].min(), dict_z[zname].max(), 10), cmap=colors[iclass])

        colors = 'grey', 'blue', 'red', 'orange', 'purple'
        for iclass in range(len(list_classes)):
            ind_class          = y_train == iclass
            plt.scatter(X_lda[ind_class, 0], X_lda[ind_class, 1], s=30, color=colors[iclass], edgecolors='k', label = list_classes[iclass])
...
def process_data(sess_filename, type_classes):
    # Parâmetros do dado
    Fs           = 512
    b, a         = signal.butter(4, (6, 40), 'bandpass', fs=Fs)
    ...
    if type_classes == '2classes':
        ...
        list_classes = ['baseline', 'left_hand', 'right_hand', 'left_elbow', 'right_elbow']
        nClasses = 5
    ...
    best_c_mean                = run_cv_return_c_mean(cov_data_mi, labels, trials, unique_labels, list_classes, n_folds = 10)
    ...
    for iclass in range(nClasses):
        kde_name           = 'kde' + str(iclass)
        zname              = 'z' + str(iclass)
        dict_kde[kde_name] = gaussian_kde(X_lda[labels == iclass].T)
        dict_z[zname]      = dict_kde[kde_name].evaluate(np.vstack([xx.ravel(), yy.ravel()])).reshape(xx.shape)
        plt.contourf(xx, yy, dict_z[zname], alpha=0.5, levels=np.linspace(dict_z[zname].min(), dict_z[zname].max(), 10), cmap=colors[iclass])
