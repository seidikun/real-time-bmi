...
def run_cv_return_c_mean(data, labels, all_trial_num, n_folds = 20):
    # Init variables
    best_acc      = 0
    best_c_mean   = None
...
if type_classes == 'free':
    ...
    data, labels, all_trial_num = data_preparation(data_eeg, a, b, columns, dict_inds, dict_labels, epoch_size, overlap_step)
    unique_labels               = np.unique(all_trial_num)
    best_c_mean                 = run_cv_return_c_mean(data, labels, all_trial_num, n_folds = 20)
