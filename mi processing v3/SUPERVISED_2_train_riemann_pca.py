...
def data_preparation(data_eeg, a, b, columns, left_inds, right_inds, epoch_size):
  # Define parâmetros
  count_files  = 0    # contador de arquivos
  overlap_step = 0.625  # tamanho do passo para sobreposição de janelas
...
data, labels, all_trial_num = data_preparation(data_eeg, a, b, columns, left_inds, right_inds, epoch_size)
...
kde0 = gaussian_kde(X_pca[labels == 0].T)
kde1 = gaussian_kde(X_pca[labels == 1].T)
...
with open(sess_filename + '_kde0.pkl', 'wb') as file:
    pickle.dump(kde0, file)
with open(sess_filename + '_kde1.pkl', 'wb') as file:
